...
/*   Updated: 2024/04/13 11:36:49 by packmanich       ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	move_if_not_wall(int off_x, int off_y, t_map *map_data)
{
	...
	x = map_data->player_img->instances[0].x / TILESIZE + off_x;
	y = map_data->player_img->instances[0].y / TILESIZE + off_y;
	iswall = false;
	if (map_data->map[y][x] == '1')
		iswall = true;
	if (!iswall)
	{
		map_data->player_img->instances[0].x += (TILESIZE * off_x);
		map_data->player_img->instances[0].y += (TILESIZE * off_y);
		map_data->moves++;
...
static void	check_endgame(t_map *map_data)
{
	...
	x = map_data->player_img->instances[0].x / TILESIZE;
	y = map_data->player_img->instances[0].y / TILESIZE;
	isexit = false;
...
static void	collect(int collectible_index, t_map *map_data)
{
	...
	mlx_set_instance_depth
		(&(map_data->collectible_img->instances[collectible_index]), 0);
}
...
static void	handle_collectible(t_map *map_data)
{
	...
	{
		if (map_data->collectible_img->instances[collectible_index].z == 2)
